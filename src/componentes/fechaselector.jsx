...
const CarritoCompleto = ({ isOpen, onClose, idCliente, forceRefresh }) => {
    ...
    // ✅ FUNCIÓN CORREGIDA: obtenerCarritosPorFecha con mejor retorno de datos
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    ...
                    setCarritosPorFecha(emptyMap);
                    return emptyMap; // ✅ Retornar el Map vacío
                }
                throw new Error(`Error ${response.status}: ${response.statusText}`);
            }
            ...
            // ✅ Filtrar carritos pendientes Y futuros
            ...
            // ✅ Crear Map que puede contener múltiples carritos por fecha
            ...
            // ✅ Ordenar carritos por ID (más reciente primero - ID mayor)
            ...
            return carritosPorFechaMap; // ✅ Retornar el Map actualizado
            ...
            const emptyMap = new Map();
            setCarritosPorFecha(emptyMap);
            return emptyMap; // ✅ Retornar Map vacío en caso de error
        } finally {
            setLoading(false);
        }
    };
    ...
    // ✅ FUNCIÓN CORREGIDA: handleFechaChange con mejor manejo asíncrono
    const handleFechaChange = async (nuevaFecha) => {
        try {
            ...
            // ✅ CAMBIO PRINCIPAL: Esperar un poco y luego recargar carritos si es necesario
            // Esto asegura que los datos estén actualizados después de crear un turno
            await new Promise(resolve => setTimeout(resolve, 100));

            // ✅ Recargar carritos para asegurar datos frescos
            const carritosFrescos = await obtenerCarritosPorFecha();

            // ✅ Usar un pequeño delay para asegurar que el estado esté actualizado
            await new Promise(resolve => setTimeout(resolve, 50));

            // Obtener carritos de esa fecha (ahora con datos frescos)
            const carritosDeEsteFecha = carritosFrescos?.get(nuevaFecha) || [];
            console.log('🛒 Carritos encontrados para esta fecha:', carritosDeEsteFecha);

            if (carritosDeEsteFecha.length > 0) {
                // ✅ Tomar el carrito MÁS RECIENTE (primero en el array ordenado)
                ...
                // ✅ CAMBIO: Asegurar que obtenerTurnosCarrito se ejecute después del setCarritoSeleccionado
                await obtenerTurnosCarrito(carritoMasReciente.id);
    ...
    // ✅ FUNCIÓN CORREGIDA: refrescarDatos más robusta
    const refrescarDatos = async () => {
        console.log('🔄 CarritoCompleto - Refrescando datos por solicitud externa');
        
        try {
            // Limpiar estados
            const fechaAnterior = fechaSeleccionada; // Guardar la fecha que estaba seleccionada
            setFechaSeleccionada(null);
            setCarritoSeleccionado(null);
            setServicios([]);
            setAplicaDescuento(false);
            
            // Recargar datos y esperar a que terminen
            const nuevosCarritos = await obtenerCarritosPorFecha();
            
            // ✅ Si había una fecha seleccionada antes, intentar mantenerla si aún está disponible
            if (fechaAnterior && nuevosCarritos && nuevosCarritos.has(fechaAnterior)) {
                console.log(`🔄 Restaurando fecha seleccionada: ${fechaAnterior}`);
                // Usar setTimeout para asegurar que el estado se actualice correctamente
                setTimeout(() => {
                    handleFechaChange(fechaAnterior);
                }, 100);
            } else if (nuevosCarritos && nuevosCarritos.size > 0) {
                // Si no se puede restaurar la fecha anterior, seleccionar la primera disponible
                const primeraFecha = Array.from(nuevosCarritos.keys())[0];
                console.log(`🔄 Seleccionando primera fecha disponible: ${primeraFecha}`);
                setTimeout(() => {
                    handleFechaChange(primeraFecha);
                }, 100);
            }
        } catch (error) {
            console.error('❌ Error en refrescarDatos:', error);
    ...
    // ✅ EFECTO CORREGIDO: Manejo mejorado cuando se abre el modal
    useEffect(() => {
        if (isOpen && idCliente) {
            try {
