...
const Servicios = () => {
  ...
  useEffect(() => {
    const fetchCategorias = async () => {
      try {
        const response = await fetch('https://spabackend-production-e093.up.railway.app/api/servicios/categorias');
        ...
        await Promise.all(data.map(async (categoria) => {
          const servResponse = await fetch(`https://spabackend-production-e093.up.railway.app/api/servicios/por-categoria/${categoria.id_categoria}`);
